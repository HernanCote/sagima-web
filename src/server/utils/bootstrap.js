...
async function transverseDir(dir, root) {
  ...
  try {
    ...
    logger.error(`Error getting path: ${dir}`, err.toString());
    logger.trace(err);
  }
...
async function loadControllers(controllerDir) {
  ...
  paths.forEach(path => {
    try {
      ...
      logger.info('🎮 ', `Loaded controller "${toTitleCase(parsedPath)}"`);
    } catch (err) {
      logger.trace(err);
    ...
    .forEach(path => {
      try {
        ...
        if (middleware.loadOnInit) {
          ...
          logger.info('📦 ', `Loaded global middleware "${parsedPath}"`);
        } else {
          ...
          logger.info('📦 ', `Loaded middleware "${parsedPath}"`);
        }
      } catch (err) {
        logger.trace(err);
...
async function loadRoutes(
  ...
  paths.forEach(path => {
    ...
    try {
      ...
      logger.info('🌐 ', `Mapped {${route}} route`);
    } catch (err) {
      logger.trace(err);
