...
const getFullMargin = isFull => (isFull ? `calc(-2 * ${theme.grid.itemsPadding})` : 0);

const Root = styled.div`
	display: flex;
	flex-wrap: wrap;
	margin: {
          ${({ fullHeight }) => getFullMargin(fullHeight)}
...
const Grid = ({
  className, children, itemsPerRow, fullWidth, fullHeight,
}) => (
  <Root
    className={className}
