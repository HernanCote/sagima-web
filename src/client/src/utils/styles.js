...
const mediaMinWidth = size =>
  `@media only screen and (min-width: ${toPixel(size)})`;
const getMediaMinWidth = size =>
  mediaMinWidth((theme.breakpoints && theme.breakpoints[size]) || size);
const toCSSURL = url =>
  !url ? 'none' : `url("${url.replace(/(\(|\))/g, (_, g1) => escape(g1))}")`;
